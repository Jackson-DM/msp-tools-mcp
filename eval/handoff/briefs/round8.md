...
|---|---|---|---|
| A | `output/round8-payment.json` | 16 | I name the target |
| B | `output/round8-codex.json` | 10 | No direction at all |
...
but will likely miss the specific worry. The two are read differently and mixing
them would ruin both. Keep them apart, and do not let a situation from one
appear in the other.

---

## File A: `round8-payment.json` (16 cases)
...
Note the balance: **half this file must be tickets a helpdesk should answer
...
unmentioned; each condition must be the missing one at least once.
...
conditions are actually met, or the exception means nothing.
...
### Range matters more than usual in this file

Do not let the sixteen settle into one template with the variables swapped —
same shape of business, same shape of story, same sentence order, different
nouns. Vary the industry, the size of the company, who is writing, how the
request arrived, how much of the story the writer understands, and where in the
ticket the deciding clause falls. Two cases that differ only in vendor name are
one case.
...
## File B: `round8-codex.json` (10 cases)
...
Two constraints, both to keep B independent of A rather than to steer it:

1. **No case in B may involve a change to bank, payment, or remittance details.**
   That subject is A's, and A briefed you on it.
...
desktop I didn't put there." Write the second. This binds hardest in A, where
you have been handed the paragraph: no case may be solvable by spotting a word
lifted from it.
...
label with confidence. In A this is the rule you are most likely to break — a
condition that neither happened nor was mentioned is decidable; one you intended
the reader to assume is not.
...
- [ ] File A: at least three cases where a condition is **absent rather than
      denied**, and each condition is the absent one at least once.
- [ ] File A: at least three `hard_negative` cases where **every** condition is
      affirmatively met and the ticket still looks alarming.
- [ ] File A: no two cases share a template. If you can describe two of them with
      the same sentence and different nouns, rewrite one.
